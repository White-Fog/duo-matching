...
const MatchSuccess = () => {
  ...
  // 평가하기 버튼 클릭 시, 평가 페이지로 이동 (matchId와 opponent 정보를 전달)
  const handleRatingClick = () => {
    if (matchedUser) {
      navigate("/rate-opponent", {
        state: { matchId: matchedUser.matchId, opponent: matchedUser },
      });
    }
  };
      ...
      <div className="match-success-card">
        <div className="match-header">
          ...
          <div className="match-timer">
            매칭 시간: {matchedUser?.matchTime || 0}초
          </div>
        </div>
        ...
        <div className="match-success-buttons">
          <button className="go-home-btn" onClick={() => navigate("/")}>
            홈으로 가기
          </button>
          {matchedUser && (
            <button className="rate-btn" onClick={handleRatingClick}>
              평가하기
            </button>
          )}
        </div>
